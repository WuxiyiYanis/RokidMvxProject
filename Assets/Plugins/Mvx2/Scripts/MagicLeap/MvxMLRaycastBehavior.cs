...
// Mvx2 Note: Inspired by MLRaycastBehavior from the original MagicLeap samples package
#if UNITY_LUMIN
...
namespace MVXUnity.ML
{
    ...
    public class MvxMLRaycastBehavior : MonoBehaviour
    {
        public enum Mode
        {
            World,
...
#if PLATFORM_LUMIN
                if (!Raycast(_raycastParams, HandleOnReceiveRaycast).IsOk)
                {
                   _isReady = true;
                }
#endif
